  ...
  if has_make then
    if build_type == "Debug" then
      ...
      if has_debug then
        ...
        vim.notify("No 'debug' target found in Makefile; running plain 'make' for Debug build.", vim.log.levels.WARN)
  ...
  handle = uv.spawn(bash, {
    ...
    vim.schedule(function()
      ...
      if code == 0 then
        ...
        local new_ok, new_ret = pcall(function()
          if notif then
            return notify_impl(ok_msg, vim.log.levels.INFO, { title = title, timeout = 3000, replace = notif })
          else
        ...
        local err_msg = (final_text ~= "" and final_text) or ("Build failed (exit code " .. tostring(code) .. ")")
        local new_ok, new_ret = pcall(function()
          if notif then
